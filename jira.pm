...
{ my $issueid=shift;
    ...
    if($status eq "Open" or $status ne "Dev in Progress" or $issuetype !~ /Implementation/i) { # TODO rework into updating jira ticket | notifying user
        system("echo 'jira warning: https://jira.suse.com/browse/$issueid status=$status issuetype=$issuetype' | mailx -c rtsvetkov\@suse.com -s IBS/obsbugzilla/jira bwiedemann\@suse.de");
    }
    return $data;
}
