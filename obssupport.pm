...
sub srurl(@)
{
	return join("",map {"https://build.suse.de/request/show/$_\n"} @_);
}
...
{ my($bugid, $comment)=@_;
	my $proxy=bugzillahandle;
	my $soapresult2 = $proxy->call('Bug.add_comment', {id => $bugid, comment => $comment, is_private=>1, private=>1, isprivate=>1});
...
{ my($bugid, @sr)=@_;
	...
	eval { # catch die
		...
		if(!$debug) {
			print "adding to https://bugzilla.novell.com/show_bug.cgi?id=$bugid\n$comment\n";
